...
const buttonVariants = cva(
  // Base styles: Flex layout, centering, smooth transitions, and click scaling effect
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 active:scale-95",
  {
    variants: {
      variant: {
        // Default: AI Gradient style with glow effect
        default:
          "bg-gradient-to-r from-violet-600 to-indigo-600 text-white shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:from-violet-500 hover:to-indigo-500 border-0",
        
        // Destructive: Subtle red background
        destructive:
          "bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500/20 shadow-sm",
        
        // Outline: Glassmorphism effect for secondary actions
        outline:
          "border border-input bg-background/50 backdrop-blur-sm hover:bg-accent hover:text-accent-foreground shadow-sm",
        
        // Secondary: Muted depth
        secondary:
          "bg-secondary text-secondary-foreground hover:bg-secondary/80 shadow-sm border border-transparent",
        
        // Ghost: Clean hover state
